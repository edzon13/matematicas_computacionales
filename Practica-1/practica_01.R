...
hiperbola <- function(h, k, a, b, horizontal){
  ...
  if (horizontal){ # hiperbola sobre el eje x
    ...
    yderpositiva <- k + sqrt((b^2/a^2)*((xder - h)^2) - b^2) # parte positiva del dominio izquierdo
    ydernegativa <- k - sqrt((b^2/a^2)*((xder - h)^2) - b^2) # parte negativa del dominio izquierdo
    ...
    xderpositiva <- h + sqrt((b^2/a^2)*((yder - k)^2) - b^2) # parte positiva del rango inferior
    xdernegativa <- h - sqrt((b^2/a^2)*((yder - k)^2) - b^2) # parte negativa del rango superior
    # graficamos
    ...
    points(x = c(h), y = c(k+(a+c)), col = "red") 
  }
}
